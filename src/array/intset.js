 ...
 */
export function dedupSortedArr(arr) {
 ...
 */
export function hasNoDups(arr /*sorted*/) {
	if( !arr ) return false
	let len=arr.length-1;
	for(let i=0;i<len;i++){
		if(arr[i] > arr[i+1]) throw new Error("arr (param) is not sorted");
 ...
 */
export function isSorted(arr /* may not be sorted sorted*/) {
	let len=arr.length-1;
	for(let i=0;i<len;i++){
		if(arr[i]>arr[i+1]){
			return  false;
 ...
 */
export function asSortedSet(array) {
	let arr;
 ...
 */
export function similarity(sortedA, sortedB) {
	let j=0;
 ...
 * @return {[Array]}         sorted array with no duplicates (a set)
 */
export function intersect(sortedL/*list od sets*/) {
 ...
 */
export function intersectL(lsa) {
...
export function max(list) { return (!list || list.length == 0) ? undefined : list[list.length-1];}
...
/**
 * Union of a list of sets, passed in as arguments: assumes both sets are are represented by sorted array, and as per set definition there are no dups 
 * @param 	{[Array]} list : array[ array{set}... ] - array{set} of sorted elements (ascending order) and no duplicated
 ...
 * @return {[set]} a new set, where result = lsa[0] ∪ lsa[1] ∪ ...
 */
export function unionL(lsa /* array of sets */) {
 ...
 */
export function subtract(fromSet,elementsToRemove) {
	let res = [];
	let j=0;
 ...
 */
export function findIndexOf(aSet, searchElement) { 
	let minIndex = 0;
